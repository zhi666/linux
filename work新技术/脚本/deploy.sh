...
#安装软件
yum install -y vim lsof wget iptables* bc openresty-resty openresty openresty-opm fail2ban unzip google-authenticator zabbix-agent expect lrzsz iftop net-tools rsync
...
#zabbix监控nginx
mkdir /usr/local/openresty/nginx/conf/conf.d -p
cat <<EOF >/usr/local/openresty/nginx/conf/conf.d/nginx_status.conf
server {
        listen       80 ;
